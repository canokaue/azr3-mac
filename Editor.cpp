...
#define IDB_cknob           "cknob.bmp"
#define IDB_vonoff          "vonoff.bmp"
#define IDB_dbbrown         "dbbrown.bmp"
#define IDB_dbwhite         "dbwhite.bmp"
#define IDB_inside          "inside.bmp"
#define IDB_miniledred      "miniledred.bmp"
#define IDB_minioffon       "minioffon.bmp"
#define IDB_yellow          "num_yellow.bmp"
#define IDB_onoffgreen      "onoffgreen.bmp"
#define IDB_dbblack         "dbblack.bmp"
#define IDB_panelfx         "panelfx.bmp"
#define IDB_panelvoice      "panelvoice.bmp"
#define IDB_vu              "make a change.bmp"
/*
enum
{
	IDB_cknob = 128,
...
void Editor::idle()
{
	if(ctrl_splash->getValue() && !ctrl_splash->isDirty())
		return;
	...
	if(justloaded && ctrl_display)
	{
		if(ticks<100)
		{
			...
			ctrl_display->setstring(1,"=== Rumpelrausch Taips ===");
			ctrl_display->setstring(2,"Loading Rumpel/OS V3");
			ctrl_display->setstring(3,"");
		}
		else if(ticks>1000)
		{
			//effect->setParameter(n_mute,0);		// moved to process
...
//-------- enumerated controls
	...
	controls[c]=new CFader(rct,this, c,bitmaps[b_dbbrown],bitmaps[b_panelfx],31);
	frame->addView(controls[c]);
	ctyp[c]=cCFader;
...
//------ voice params ---------------------------------------
	...
	controls[c]=new COnOffButton(rct,this, c,bitmaps[b_onoffgreen]);
	frame->addView(controls[c]);
	ctyp[c]=cCOnOffButton;
...
void Editor::valueChanged (CControl* control)
{
	if(!control)
		return;
...
void Editor::c_visible(long tag,bool show)
{
	...
	if(show)
		...
		frame->removeView(control, false);	//Migrating from VSTGUI 2.3 to VSTGUI 3.0
											//CFrame::removeView (CView *pView, const bool &withForget = true)	
											//The second parameter (withForget) has changed its default parameter !!!!
}